...
function startAmbient(){
  try{
    ...
    notes.forEach((freq,i)=>{
      const osc=ambient.createOscillator();
      const gain=ambient.createGain();
      osc.type="sine";
      osc.frequency.value=freq;
      gain.gain.setValueAtTime(0,ambient.currentTime+i*1.3);
      gain.gain.linearRampToValueAtTime(.24,ambient.currentTime+i*1.3+.35);
      gain.gain.exponentialRampToValueAtTime(.001,ambient.currentTime+i*1.3+1.25);
      osc.connect(gain).connect(master);
      osc.start(ambient.currentTime+i*1.3);
      osc.stop(ambient.currentTime+i*1.3+1.3);
    });
    audioOn=true;
